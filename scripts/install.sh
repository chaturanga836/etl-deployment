...
build_install_frontend_image() {
  local ef="$1"
  # Default: on-host build with public Keycloak/API URLs from wizard .env.
  # Registry images bake localhost and break login (BUILD_INSTALL_FRONTEND=false
  # only for emergency pull of GHCR — not for customer installs).
  if [[ "${BUILD_INSTALL_FRONTEND:-true}" != "true" ]]; then
    # shellcheck source=lib/frontend-install-build.sh
    source "${ROOT_DIR}/scripts/lib/frontend-install-build.sh"
    frontend_install_restore_registry_frontend_image "$ef" || true
    echo "WARN: BUILD_INSTALL_FRONTEND=false — using registry FRONTEND_IMAGE." >&2
    echo "WARN: Registry images redirect login to localhost:8081." >&2
    return 0
  fi
  # shellcheck source=lib/frontend-install-build.sh
  source "${ROOT_DIR}/scripts/lib/frontend-install-build.sh"
