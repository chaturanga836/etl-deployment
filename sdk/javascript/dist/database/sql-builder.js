...
function buildUpdateSql(schemaName, tableName, columns, originalRow, updatedRow) {
    const tableRef = qualifiedTable(schemaName, tableName);
    ...
    const whereParts = primaryKeyColumns(columns).map((key) => `${quoteIdent(key)} = ${sqlLiteral(originalRow[key])}`);
    return `UPDATE ${tableRef}\nSET ${setParts.join(", ")}\nWHERE ${whereParts.join(" AND ")};`;
}
function buildDeleteSql(schemaName, tableName, columns, row) {
    const tableRef = qualifiedTable(schemaName, tableName);
    const whereParts = primaryKeyColumns(columns).map((key) => `${quoteIdent(key)} = ${sqlLiteral(row[key])}`);
