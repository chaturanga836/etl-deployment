...
| Login → `localhost:8081` | `scripts/lib/frontend-install-build.sh`, `scripts/install.sh`, `elt-frontend/src/lib/keycloak.ts` — then **fresh install** (on-host frontend build), not rebuild mid-flight |
| Frontend build: `path ".../elt-frontend" not found` | Wizard `docker build` via socket can’t see host paths — stream context in `frontend-install-build.sh` (`tar \| docker build -`). Also check disk: `df -h /`, `docker builder prune -af` |
