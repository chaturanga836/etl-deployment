...
from elt_cli.client import EltClientError, apply_migrations, fetch_applied_versions, get_access_token, make_client
from elt_cli.config import (
    DEFAULT_CONFIG_DIR,
...
from elt_cli.migrations import migration_status_rows, pending_migrations, scan_local_migrations
...
@click.group()
def cli() -> None:
    """ELT Engine CLI — database migrations for workspace databases."""


@cli.command()
@click.option("--path", "root", type=click.Path(path_type=Path), default=".", show_default=True)
def init(root: Path) -> None:
    """Create elt/ with config.toml and migrations/ directory."""
    ...
    click.echo("Next: run `elt link` to connect to your workspace database.")


@cli.command()
@click.option("--api-url", required=True, help="ELT API base URL (e.g. https://api.example.com)")
...
def link(api_url: str, workspace_id: int, database_id: int, root: Path) -> None:
    """Save API URL and workspace/database targeting to elt/config.toml."""
    config_dir = root / DEFAULT_CONFIG_DIR
    if not config_dir.exists():
        click.echo("Run `elt init` first.", err=True)
        raise SystemExit(1)
...
def migration_new(name: str, root: Path) -> None:
    """Create a new timestamped migration file under elt/migrations/."""
...
def db_push(root: Path, yes: bool, dry_run: bool) -> None:
    ...
    if not get_access_token():
        click.echo("Set ELT_ACCESS_TOKEN to a valid Keycloak bearer token.", err=True)
        raise SystemExit(1)

    local = scan_local_migrations(migrations_dir(root))
    if not local:
        click.echo("No migration files in elt/migrations/.")
        return
